...
from .adapters import AnkiConnectGateway, AppleNotesGateway, JMDictDictionarySource
from .anki import (
    DEFAULT_DECK,
    DEFAULT_ENDPOINT,
    DEFAULT_NOTE_TYPE,
...
from .anki_sync_service import AnkiSyncConfig, AnkiSyncService
from .gateways import NotesGateway
...
def build_parser() -> argparse.ArgumentParser:
    ...
    download = commands.add_parser(
        "download-dictionary", help="download the latest English JMDict JSON with examples"
    )
    ...
    sync = commands.add_parser(
        "sync", help="create cards directly in Anki Desktop through AnkiConnect"
    )
    ...
    sync.add_argument(
        "--dry-run", action="store_true", help="show what would be added without changing Anki"
    )
...
def _load_words(
    args: argparse.Namespace, parser: argparse.ArgumentParser, notes_gateway: NotesGateway
) -> list[str]:
    ...
    if args.words_file:
        return [
            line.strip()
            for line in args.words_file.read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
...
def _run(
    ...
    notes_gateway: NotesGateway,
    anki_sync_service: AnkiSyncService,
) -> int:
    ...
    if args.command == "anki-check":
        ...
        print(
            f"AnkiConnect {version} is ready for deck '{args.deck}' and note type '{args.note_type}'"
        )
        return 0
    if args.command == "anki-setup":
        version = anki_sync_service.ensure_config(args.deck, args.note_type)
        print(
            f"AnkiConnect {version} is configured for deck '{args.deck}' and note type '{args.note_type}'"
        )
        return 0
    ...
    if args.command == "sync":
        ...
        card_count, missing = export_cards_from_words(
            words, args.dictionary, args.output, dictionary_source
        )
